...
HRESULT __stdcall eDirectX11Hook::Present(IDXGISwapChain * pSwapChain, UINT SyncInterval, UINT Flags)
{
	...
	if (draw)
	{
		...
		Notifications->PushNotification("MKXHook %s is running! Press F1 (or L3+R3 on a controller if controller support enabled) to open the menu. Build date: %s\n", MKXHOOK_VERSION, __DATE__);
		draw = false;
	}
